...
| `package_license` | `LICENSE` file contents and `license` field/classifiers; map to MIT / Apache-2.0 / BSD-3-Clause, else Proprietary |
...
- **`pyproject.toml`** — start from the rendered one and port the project’s reality into
  ...
  as below). Keep the rendered `[build-system]` (hatchling + uv-dynamic-versioning),
  `[tool.uv]`, `[tool.ruff]`, `[tool.basedpyright]`, `[tool.pytest.ini_options]`
  sections.
- **Code layout** — move code to `src/<module>/` (the convention; only keep a flat
  layout if the user insists — then adjust `packages` and `testpaths`). Ensure
  `py.typed` exists in the package.
  Tests go in `tests/`.
...
- Dynamic versioning needs a tag: if the package is on PyPI at version X.Y.Z, tag the
  ...
  project starts at `v0.1.0`. Until a tag exists, builds get a dev version — fine for
  CI, wrong for release.
