...
# Format all Markdown docs, including *.md.jinja templates. Skip tool-managed
# dirs (.tbd, .claude, .agents); gitignored paths like attic/ are skipped too.
FLOWMARK_ARGS := --auto --extend-include '*.md.jinja' \
	--extend-exclude '.tbd/' --extend-exclude '.claude/' --extend-exclude '.agents/'
